...
static bool g_uefi_backend_active = false;
...
int main() {
    ...
    while (g_settings.memory_backend == -1) {
        ...
        if (backend == 3) {
            printf("\n=== Kernel driver /w kdmapper Requirements ===\n");
    ...
    if (!g_offsets.load("offsets/offsets.json", "offsets/client_dll.json")) {
        ...
        cleanup_on_exit(); CoUninitialize(); return 1;
    }
    ...
    printf("[+] %s = menu | %s = master toggle | %s = exit\n",
           vk_name(g_settings.key_menu), vk_name(g_settings.key_master), vk_name(g_settings.key_exit));
    ...
    while (g_running) {
        ...
        if (GetAsyncKeyState(g_settings.key_master) & 1) g_settings.master_switch = !g_settings.master_switch;
        ...
        if (g_settings.aimbot_enabled || g_settings.triggerbot_enabled) {
            ...
            af.local_x     = state.local.x; af.local_y = state.local.y; af.local_z = state.local.z;
            af.local_team  = state.local.team; af.local_pawn = state.local.pawn;
            af.screen_w    = g_overlay.width; af.screen_h = g_overlay.height;

            if (state.local.camera.valid) {
                af.eye_origin   = state.local.camera.origin;
                af.view_angles  = state.local.camera.angles;
                af.camera_fov   = state.local.camera.fov; af.camera_valid = true;
            } else {
                af.eye_origin   = { state.local.x, state.local.y, state.local.z + 64.0f };
                af.camera_fov   = 90.0f; af.camera_valid = false;
            }

            for (int i = 1; i < EntityList::MAX_PLAYERS; i++) {
                const auto& p = state.players[i];
                af.targets[i].valid      = p.valid; af.targets[i].team = p.team; af.targets[i].health = p.health;
                af.targets[i].head_pos   = p.head_world; af.targets[i].neck_pos = p.neck_world;
                af.targets[i].chest_pos  = p.chest_world; af.targets[i].pelvis_pos = p.pelvis_world;
            }
        ...
        float view_pitch_deg = -asinf(std::clamp(fwd_z, -1.0f, 1.0f)) * 180.0f / 3.14159265f;
        float view_yaw_deg   =  atan2f(fwd_y, fwd_x) * 180.0f / 3.14159265f;
