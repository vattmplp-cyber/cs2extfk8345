...
// Чіткий відносний шлях, щоб MSVC бачив інтерфейс пам'яті
...
private:
    ...
    bool parse_offsets(
        ...
        try {
            ...
            C_BaseEntity.m_iTeamNum =
                cs["C_BaseEntity"]["fields"]["m_iTeamNum"];

            C_BaseEntity.m_pGameSceneNode =
                cs["C_BaseEntity"]["fields"]["m_pGameSceneNode"];

            C_BaseEntity.m_iHealth =
                cs["C_BaseEntity"]["fields"]["m_iHealth"];

            CGameSceneNode.m_vecAbsOrigin =
                cs["CGameSceneNode"]["fields"]["m_vecAbsOrigin"];

            CSkeletonInstance.m_modelState =
                cs["CSkeletonInstance"]["fields"]["m_modelState"];
            ...
            C_CSPlayerPawn.m_bIsScoped =
                cs["C_CSPlayerPawn"]["fields"]["m_bIsScoped"];

            C_BaseModelEntity.m_vecViewOffset =
                cs["C_BaseModelEntity"]["fields"]["m_vecViewOffset"];

            C_CSPlayerPawnBase.m_pWeaponServices =
                cs["C_BasePlayerPawn"]["fields"]["m_pWeaponServices"];

            CPlayer_WeaponServices.m_hActiveWeapon =
                cs["CPlayer_WeaponServices"]["fields"]["m_hActiveWeapon"];
            ...
            C_CSPlayerPawn.m_entitySpottedState =
                cs["C_CSPlayerPawn"]["fields"]["m_entitySpottedState"];

            EntitySpottedState_t.m_bSpotted =
                cs["EntitySpottedState_t"]["fields"]["m_bSpotted"];
    ...
    TestResult functional_test() {
        ...
        uintptr_t entity_list =
            g_memory->read<uintptr_t>(
                client_base + client.dwEntityList
            );

        if (!entity_list) {
            printf(
                "[!] entity_list is null (offset 0x%X)\n",
                client.dwEntityList
            );
            return TestResult::OFFSETS_WRONG;
        }
        ...
        uintptr_t first_page =
            g_memory->read<uintptr_t>(entity_list + 16);
        ...
        for (int i = 1; i < 64; i++) {
            ...
            int team =
                g_memory->read<int>(
                    pawn + C_BaseEntity.m_iTeamNum
                );

            int health =
                g_memory->read<int>(
                    pawn + C_BaseEntity.m_iHealth
                );
        ...
        if (valid_players > 0 &&
            bogus_reads <= valid_players) {

            printf(
                "[+] Found %d valid players in functional test\n",
                valid_players
            );
        ...
        printf(
            "[!] Functional test failed: %d valid, %d bogus reads\n",
            valid_players,
            bogus_reads
        );
    ...
    bool run_dumper() {
        ...
        if (!std::filesystem::exists(dumper)) {
            ...
            printf(
                "[!] Place next to this executable and restart.\n"
            );
        ...
        if (!std::filesystem::exists(dump_dir)) {
            printf(
                "[!] cs2-dumper output directory '%s' not found\n",
                dump_dir
            );
        ...
        for (const auto& fc : needed) {
            if (std::filesystem::exists(fc.src)) {
                ...
                printf(
                    "[!] Expected file %s not found in dumper output\n",
                    fc.src
                );
